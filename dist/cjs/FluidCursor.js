...
exports.FluidCursor = void 0;
require("./css/FluidCursorStyle.scss");
const CursorComponent_1 = require("./CursorComponent");
const TargetComponent_1 = require("./TargetComponent");
const Vectors_1 = require("./Vectors");
const IntersectionType_1 = require("./IntersectionType");
const gsap_1 = require("gsap");
class FluidCursor {
    constructor() {
        this.lastMouse = new Vectors_1.Vector2(0, 0);
        this.targets = [];
        this.cursors = [];
        ...
        gsap_1.default.ticker.add(() => { this.animateCursors(); });
    }
    getCursorPosition(event) {
        ...
        return new Vectors_1.Vector2(mouseX, mouseY);
    }
    addGSAPPositionTweens(cursor, position) {
        ...
        const tween = gsap_1.default.to(cursor.position, {
            x: position.x,
            y: position.y,
    ...
    addGSAPRotationTweens(cursor, delta) {
        ...
        const tween = gsap_1.default.to(cursor.rotation, {
    ...
    checkIntersection(e) {
        ...
        for (const target of this.targets) {
            ...
            if (intersection && !target.hasIntersection)
                type = IntersectionType_1.default.ENTER;
            else if (intersection && target.hasIntersection)
                type = IntersectionType_1.default.DORMANT;
            else if (!intersection && target.hasIntersection)
                type = IntersectionType_1.default.LEAVE;
            if (!type)
                continue;
    ...
    updateTargetMorphCursor(e) {
        for (const target of this.targets) {
            if (!target.morphCursor)
                continue;
            target.morphTo(new Vectors_1.Vector2(e.clientX, e.clientY));
        }
    }
    setLastMouse(e) {
    ...
    setUpFluidTarget(target) {
        this.targets.push(new TargetComponent_1.default({
            HTML: target,
    ...
    setUpFluidCursor(cursor) {
        this.cursors.push(new CursorComponent_1.default({
            HTML: cursor,
...
exports.FluidCursor = FluidCursor;
