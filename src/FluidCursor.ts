...
class FluidCursor {
  ...
  constructor() {
    ...
    for (const cursor of HTMLCursorComponents) this.setUpFluidCursor(cursor);
    for (const target of HTMLTargetComponents) this.setUpFluidTarget(target);
  ...
  addGSAPPositionTweens(cursor: CursorComponent, position: Vector2) {
    if (cursor.hasIntersection() && (cursor.morphTarget || cursor.magneticTarget)) return;
    ...
    const tween = gsap.to(cursor.position, {
      ...
      ease: "power3.out",
      onUpdate: () => {
        if (cursor.hasIntersection() && (cursor.morphTarget || cursor.magneticTarget)) tween.kill();
      },
    });
  }
  
  addGSAPRotationTweens(cursor: CursorComponent, delta: number) {
    if (cursor.hasIntersection() && (cursor.morphTarget || cursor.magneticTarget)) return;
  ...
  updateCursorPosition(e: MouseEvent | Event) {
    for (const cursor of this.cursors) this.addGSAPPositionTweens(cursor, this.getCursorPosition(e));
  }
  
  updateCursorRotation(e: MouseEvent) {
    for (const cursor of this.cursors) {
      if (!cursor.rotationPhysics) return; // Assuming this is meant for checking physics
  ...
  updateCursorIntersection(type: IntersectionType, target: TargetComponent) {
    for (const cursor of this.cursors) cursor.intersection(type, target);
  }
  
  checkIntersection(e: MouseEvent | Event) {
    ...
    for (const target of this.targets) {
      ...
      let type: IntersectionType | undefined;
      
      if (intersection && !target.hasIntersection) type = IntersectionType.ENTER;
      else if (intersection && target.hasIntersection) type = IntersectionType.DORMANT;
      else if (!intersection && target.hasIntersection) type = IntersectionType.LEAVE;
  ...
  updateTargetMorphCursor(e: MouseEvent) {
    for (const target of this.targets) {
      if (!target.morphCursor) continue;
      
      target.morphTo(new Vector2(e.clientX, e.clientY));
    }
  }
  
  setLastMouse(e: MouseEvent) {
  ...
  animateCursors() {
    ...
    if (this.targets.length > 0) window.addEventListener("scroll", this.checkIntersection);
    if (this.targets.length > 0) window.addEventListener("mousemove", this.checkIntersection);
    
    for (const cursor of this.cursors) cursor.setHTML();
  }
  
  setUpFluidTarget(target: Element) {
  ...
  setUpFluidCursor(cursor: Element) {
    this.cursors.push(new CursorComponent({
      HTML: cursor as HTMLElement,
